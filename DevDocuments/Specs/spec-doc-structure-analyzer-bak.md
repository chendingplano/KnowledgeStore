## Summary
This Go program uses an LLM to analyze the document structures from an input file.

- Language: Go
- Implementation target: `ChenWeb/server/api/doc-processing/doc-structure-analyzer.go`
- Main inputs: `record_id`, `input_file` buffer, and `input_filename`
...
- FILE_BLOCK_SIZE: the block size (see below) in number of pages
