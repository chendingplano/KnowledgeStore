...
- This is a service. It subsribes from the JetStream service.
...
- 
The service monitors the database table 'kb.inputs' (refer to 'KnowledgeStore/database-table-schemas/table-schema-kb-input.md' for information about the table). It handles only the records with `type` = 'pdf'. If its status does not have an entry whose `operation` is "parse" or "parsing", it will pick up the record and start parsing the doc (i.e., extracting its text and document structure and save the results in the corresponding directory (see below). For more information about status management, refer to "Status Management" section.
