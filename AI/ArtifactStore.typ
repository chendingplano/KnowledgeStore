...
on the data the 'app' was not designed for. 
...
"My Knowledge Base Project (MKBP)", should we still use a file-based solution, but a faked one so
...
“just files.” That is the cleanest direction for MKBP.
...
So for MKBP, the top-level abstraction should be:
...
- `artifact_id` UUID
- `logical_path` string, like `/projects/mkbp/specs/vision.md`
- `artifact_type` enum
- `title`
- `mime_type`
- `current_version_id`
...
Most artifacts are immutable content snapshot. Fields include:
...
```json
{
  "logical_path": "/projects/mkbp/notes/vision.md",
  "artifact_type": "markdown",
  "title": "Vision",
  "content": "# Vision\n...",
  "metadata": {
    "tags": ["mkbp", "architecture"]
  }
}
```
  ...
  "results": [
    {
      ...
      "logical_path": "/projects/mkbp/architecture.md",
...
```json
{
  "query": "How should MKBP handle versioning?",
  "filters": {
    "collections": ["/projects/mkbp"]
  },
  "limit": 8
...
Use a simple style like `mkbp <noun> <verb>` or `mkbp <verb>`.
...
```bash
mkbp add ./vision.md --path /projects/mkbp/notes/vision.md
mkbp import https://example.com/spec --path /sources/web/spec-001
```

Create/edit

```bash
mkbp new note /projects/mkbp/notes/ideas.md
mkbp edit /projects/mkbp/notes/ideas.md
mkbp publish /projects/mkbp/notes/ideas.md
```

Read/open

```bash
mkbp open /projects/mkbp/notes/vision.md
mkbp cat /projects/mkbp/notes/vision.md
mkbp show a1
mkbp versions /projects/mkbp/notes/vision.md
mkbp diff /projects/mkbp/notes/vision.md --from v2 --to v3
```

Search

```bash
mkbp search "artifact versioning"
mkbp search "citation model" --type markdown --collection /projects/mkbp
mkbp related /projects/mkbp/notes/vision.md
```

Cite

```bash
mkbp cite /projects/mkbp/architecture.md --section "Versioning"
mkbp cite chunk c45
```
...
```bash
mkbp entity resolve "Postgres"
mkbp links /projects/mkbp/architecture.md
mkbp link /projects/mkbp/architecture.md /topics/versioning --predicate about
```
...
```bash
mkbp jobs
mkbp reindex /projects/mkbp/architecture.md
mkbp doctor
```
...
- full-text search over normalized chunks
- vector search over chunk embeddings
- combine results with simple fusion
- rerank top 20 with a lightweight reranker if available
...
```json
{
  "why_matched": {
    ...
    "filters_applied": ["collection:/projects/mkbp"]
  }
}
```
...
> “What is our current decision on artifact versioning in MKBP?”
...
- it’s probably somewhere under the MKBP project docs
...
```json
search_knowledge({
  "query": "artifact versioning current decision MKBP",
  "filters": {
    "collections": ["/projects/mkbp"]
  },
  "limit": 8
...
```text
`a17 /projects/mkbp/architecture.md`

  - snippet: “Artifacts have immutable versions...”
  - chunk `c301`
`a22 /projects/mkbp/notes/versioning-brainstorm.md`

  - snippet: “Maybe overwrite latest unless in strict mode...”
  - chunk `c455`
`a35 /projects/mkbp/rfcs/rfc-003-versioning.md`

  - snippet: “Decision: every publish creates a new immutable artifact version...”
  - chunk `c801`
`a41 /projects/mkbp/meeting-notes/2026-03-29-design-review.md`
...
```json
open_artifact("/projects/mkbp/rfcs/rfc-003-versioning.md")
```
...
```json
search_knowledge({
  "query": "versioning decision superseded changed MKBP",
  "filters": {
    "collections": ["/projects/mkbp"]
  },
  "limit": 6
...
```json
open_artifact("/projects/mkbp/meeting-notes/2026-03-29-design-review.md")
...
```json
search_knowledge({
  "query": "why artifact-centric instead of SQL raw tables LLM rationale MKBP",
  "filters": {
    "collections": ["/projects/mkbp"]
  },
  "limit": 10
...
```json
open_artifact("/projects/mkbp/architecture.md")
...
```json
open_artifact("/projects/mkbp/api-principles.md")
...
```json
open_artifact("/projects/mkbp/retrieval.md")
...
```json
find_related("/projects/mkbp/architecture.md")
```

This might surface:

- `/projects/mkbp/rfcs/rfc-001-artifact-model.md`
- `/projects/mkbp/notes/llm-tool-surface.md`
...
*What user wants*
```text
> “Create a short draft note under `/projects/mkbp/notes/` summarizing the minimal V1 worth building.”
```
...
```json
search_knowledge({
  "query": "minimal V1 worth building MKBP",
  "filters": {
    "collections": ["/projects/mkbp"]
  },
  "limit": 8
...
```json
open_artifact("/projects/mkbp/roadmap.md")
...
```json
open_artifact("/projects/mkbp/api-design.md")
...
```json
{
  "artifact_id": "a99",
  "version_id": "v1",
  "logical_path": "/projects/mkbp/notes/minimal-v1-for-mkbp.md",
  "status": "draft"
}
```
...
```text
> I created a draft note at `/projects/mkbp/notes/minimal-v1-for-mkbp.md`.
>
...
Together, they show 3 different kinds of MKBP usage.
...
One MKBP design principle in my mind is: 'be natural'. Take coding for instance, when we write code, code needs
...
That is the kind of “naturalness” MKBP should support.

The major design deficiency of the above MKBP design is that it should not present only:
...
```json
{
  "path": "/projects/mkbp/rfcs/rfc-003-versioning.md",
  "kind": "artifact",
  "artifact_type": "markdown",
...
```json
{
  "path": "/projects/mkbp/architecture.md",
  "preview": {
    "headings": [
      "Goals",
      "Artifact Model",
...
Internally, collections remain organizational metadata. Externally, expose them as path nodes. So `/projects/mkbp` may be backed by:
...
- `set_working_path("/projects/mkbp")`
- then `list_path("./rfcs")`
...
{
  "path": "/projects/mkbp/rfcs/rfc-003-versioning.md",
  "trail": ["/projects", "/projects/mkbp", "/projects/mkbp/rfcs"]
}
```
...
Search with filter `/projects/mkbp`

New flow:

- `list_path("/")`
- `list_path("/projects")`
- `list_path("/projects/mkbp")`
- inspect `rfcs`
- open likely file
- verify nearby docs
- answer
...
So MKBP should support this policy:
