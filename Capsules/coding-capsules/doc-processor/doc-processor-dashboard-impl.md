...
- Any entry cast as `StatusEntry` that has a `progress` field and no final `proc_status` → active
- Most recent entry has no finalized `proc_status` / `proc-status` / `status` value → active
...
| `blocking` | Blocking |
| `structure_analyzer`, `chunking`, `extract_doc_metadata`, `extract_metrics`, `extract_provisions` | Leaf processors |
...
- `structure_analyzer`, `chunking`, `extract_doc_metadata`, `extract_metrics`, `extract_provisions` — individually togglable, toggle-all available
