...
last-modify-time: 2026-09-12T06:47:28-05:00
keywords: Product Metric Reviewer, PMR, product metrics, metric coverage, scope profile, scope tree, part hierarchy, lifecycle aspects, coverage gaps, coverage report, metric retrieval, compliance review, standards review, evidence drawer, run diff, re-run, knowledge base
---
...
You need access to ChenWeb. Product Metric Reviewer is not yet listed in the application
navigation, so ask your ChenWeb administrator for the direct link to a product's review page.

Starting a brand-new product review — creating the product's profile, building its first draft,
and running the first review — currently requires a knowledge-base operator; there is no
in-page "start a new review" form yet (see [Section 11](#11-current-implementation-boundaries)).
Once a review has been started, everything described in this manual — reading results, curating
the product's breakdown, exploring the report, and re-running the review — happens in the page
itself, with no further operator involvement needed.
...
Open the link your administrator gave you (a page at `/home3/product-metric-review` with a run
identifier). The page loads the product's name, the breakdown tree, the results, and the
...
The right-hand pane lists every metric the run found, grouped by the scope-tree node it matched.
...
- **No in-page "create a new review" flow yet.** A knowledge-base operator must create the
  product's profile and start its first run before you can open a review page for it. Everything
  after that first run is entirely self-service.
- **Not yet in the application navigation.** You need a direct link with a run identifier.
...
|---|---|
| The review page won't open | Confirm you have the correct link, including the run identifier. Ask your administrator to confirm the review was actually started. |
