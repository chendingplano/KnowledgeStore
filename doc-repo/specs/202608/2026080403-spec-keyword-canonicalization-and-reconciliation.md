...
This is the design's central model, borrowed from UMLS. **It is generic — it applies to any keyword, from any source. It has nothing to do with metrics specifically.** A metric name is only *one possible* source of an occurrence, and only once something is built to make it one (§6.4 below). This point is worth stating before anything else, because an earlier version of this section jumped straight into a metric-name example without saying so, and read as if occurrence *meant* metric names. It doesn't. The rest of this entry is more concrete than the other D-items, and includes an explicit correction to that earlier version, because the gap between what these four words mean and what actually happens in the running code is the most common source of confusion in this module.
