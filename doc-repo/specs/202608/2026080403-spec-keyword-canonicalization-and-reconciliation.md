...
>
> **Correction to this paragraph, on reconsideration: the "deliberate conservatism" explanation offered above does not survive scrutiny, and should not have been offered as "a real argument" without checking it against the rest of `TermFamily`'s own policy.** The safety a conservative normalizer would supposedly buy — protection against a silent bad auto-merge — is already fully provided by something else: `TermFamily.AutoAcceptPolicy{Enabled: false}` means no term match can *ever* auto-accept, regardless of normalization strength (§3.1 above — `Adjudicate` can only return `deferred` or `human_review` for this family). Conservative normalization isn't closing a risk that's still open; that risk is already closed by the policy, independent of the normalizer.
>
> What conservative normalization actually costs, and what the "deliberate" framing missed: **candidate recall.** `CandidateNodes` only surfaces a match when the normalized key lines up; a weak normalizer means a genuinely-duplicate term can fail to surface as a candidate at all — not flagged and correctly rejected by a reviewer, but never shown to one in the first place. For a governed catalog, silently missing a duplicate is arguably a worse failure than showing a reviewer one extra candidate to dismiss.
>
> There is also no comment or design note anywhere explaining *why* `TermFamily.Normalizer()` is "basic" — its only comment is the generic doc-comment on the method. `Normalizer{Name: "basic", ...}` with no custom `NormFunc` is exactly what results from *not writing one*. The more honest explanation: `TermFamily` (P2) predates the keyword-specific pipeline (P3), and nobody has gone back to ask what term labels actually need. This is a gap left by build order, not a reasoned choice — and stating it as "a real argument for intentional" without checking it against `Enabled: false` was itself an instance of the thing worth avoiding: giving a plausible-sounding answer instead of an actually-checked one.
>
> **The decision, restated correctly:** `TermFamily` needs its own deliberately-chosen normalizer profile — possibly still different from `KeywordFamily`'s in some respects, but chosen because it's right for term labels, not inherited by default from not having built one. This should be settled, and a real profile written, before `aligns_to_term` is built on top of it.
>
> **Two more precise findings, from tracing a concrete input ("显示亮度") through both families side by side rather than reasoning about them abstractly:**
>
> 1. **`TermFamily.Scope`'s own doc comment contradicts its own code.** The comment reads "the term family scopes by module"; the function is `func (f TermFamily) Scope(surface string) string { return "" }` — an unconditional empty string, every time, for every input. Combined with `CandidateNodes`' SQL — `WHERE ($1 = '' OR t.module_id = $1)` — an always-empty `$1` makes the module filter structurally impossible to apply: `TermFamily`'s kernel-driven candidate search runs across **every module, always**, regardless of which module a term candidate was actually proposed for. This is the same *shape* of bug as K2 (a real scope value is computed in the outer wrapper and used only to label the output, never to filter matching), independently present in the other family, and not previously documented anywhere.
> 2. **`TermFamily.AutoAcceptPolicy{Enabled: false, MinScore: 0}` leaves `MaxCandidates` at its zero value — 0 — which makes the `ambiguous` verdict structurally unreachable for this family.** `Adjudicate`'s tie-check is gated on `policy.MaxCandidates > 0`; at `0` it can never fire, so a five-way tie and a single clean match both come back `human_review`. Combined with `Enabled: false` (which makes `auto_accepted` unreachable too), `TermFamily` can only ever produce `deferred` (zero candidates) or `human_review` (any candidates at all) — never `ambiguous`, despite `Verdict` listing four options everywhere the type is documented. This may well be the intended, correct behavior for governed content (never silently disambiguate a term match) — but if so, it should say so explicitly, the same way `KeywordFamily`'s policy comment explains *why* its numbers are what they are.
>
> Concretely: given the same hypothetical data (a keyword concept and a term candidate both already existing for "显示亮度"), a realistic outcome is Keyword returning `auto_accepted` while Term returns `human_review` for an equally strong match — not because the two disagree about the evidence, but because Term's policy shape cannot express anything stronger than "needs a human" under any circumstance.
