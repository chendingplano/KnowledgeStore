...
* **2026/06/30, Report template restructured with Table of Contents and package→aspect heading
  hierarchy.** The Typst report template (`template-document-report.typ`) now includes a
  **Table of Contents** section (via `#outline()`) after the cover block. The findings section
  is restructured so that **packages are Level 1 headings** and **aspects are Level 2 headings**
  beneath their package, replacing the previous flat "Findings by Aspect" layout. The Go source
...
The PDF report is generated by rendering the skeleton into a Typst source file
via `buildTypstSource` (`typst_report.go`), which calls the Typst template
(`template-document-report.typ` via `#import`) with the report data passed as
function arguments: `doc-title`, `aspect-stats`, `aspects` (array of content
blocks), `summary`, `grounding-refs`, `supporting-refs`, etc.

The report is structured hierarchically with a **Table of Contents** (via
`outline()`) after the cover block. Findings are organized by **package
as Level 1 headings** (e.g., `#heading(level: 1, "P5 - Technical & Compliance
Standards")`) and by **aspect as Level 2 headings** beneath their package.
Packages appear in the order defined by `doc-review.local.toml` (as parsed by
`packageOrderFromTOML`), not in the numeric P1–P6 order, so added packages
appear where the config places them. Within each package, aspects appear in the
order their findings were produced by the reviewer. Each `aspect-section(...)`
call renders its own findings block, overall assessment, main problem analysis,
and guidelines/recommendations. The Typst heading styling (accent-colored blocks
for Level 1, colored text for Level 2, muted text for Level 4 sub-sections) is
defined in the template via `show heading.where(level: N)` rules.
...
- Updated: `docs/doc-templates/template-document-report.typ` — Added Table of Contents section (`outline()`) after cover; restructured aspect headings from Level 3 → Level 2; removed "Findings by Aspect" wrapper so packages render as Level 1 chapters (2026/06/30)
- Updated: `server/api/doc-reviews/typst_report.go` — `buildTypstSource` now emits `heading(level: 1, "Package Name")` per pass and groups findings by aspect within each package, generating one `aspect-section(...)` per aspect at Level 2 (2026/06/30)
