...
### `kb.pipeline_policies` scope: researched, treated as a deferred simplification

Read DR6 (ADR `2026072901-adr-ontology-platform-and-adaptive-pipeline.md`, lines 339–427) directly, per the open item above. The ADR's intended schema is materially richer than what this session actually built:

- `kb.pipeline_policies` (`policy_id, version, status, source_ref, checksum, activated_at, activated_by`) is a **versioned activation envelope** — `kb.pipeline_bindings` and `kb.pipeline_rules` both carry a `policy_id` FK, so policy changes are authored as a new version and explicitly activated, not applied live the moment a row is inserted. Nothing built this session has a `policy_id` or activation concept at all.
- `kb.pipeline_bindings` in the ADR spans `scope_kind (system|tenant|knowledge_store|user|document)` with a generic `predicate JSONB`. What exists is a single `ks_store_id` FK (`UNIQUE`, one row per store) — `scope_kind=knowledge_store` only.
- `kb.pipeline_rules` in the ADR gates **individual processors within an already-selected pipeline** via `target_processor` + `effect (require|enable|skip|defer)` — a second, finer-grained mechanism distinct from pipeline selection. What exists only *selects which pipeline* runs (via facet-matched `match_input_doc_type`/`match_source_language`/`match_knowledge_store_binding` → `pipeline_id`); there is no way to skip one processor on one document while leaving the rest of the pipeline's allowlist alone.
- `kb.pipelines` in the ADR is versioned (`name, version, ...`) and authored/compiled/activated the same way ontology modules are (DR17). What exists is a flat, unversioned row (`processors TEXT[]`, `legacy_equivalent`) with no `version`/`status`/`checksum`/`source_ref`.

Asked the user how to proceed, given the ADR's own P1 exit criteria literally says "with the seeded **policy** activated" — implying an activation concept this session didn't build. **Decision: document as a deferred simplification, not reconcile now.** The implemented schema still satisfies P1's exit bar functionally (DR7 precedence and blocking-conflict detection both work; "run pipeline A on knowledge store K1" is expressible as one `kb.pipeline_bindings` row; a deliberately conflicting binding pair fails the run; shadow mode shows the intended skip decisions) — it just does so with a simpler, unversioned, non-JSONB-predicate mechanism than DR6 describes. Versioning/activation (`kb.pipeline_policies`), `scope_kind` generalization, `predicate JSONB`, and per-processor `require|enable|skip|defer` gating in `kb.pipeline_rules` are left for a later slice, to be picked up explicitly rather than assumed done.

### P1 benchmark closeout, take two: a real enforcement bug found while building the comparison, fixed

Retried the Chunk 6 comparison after the user independently killed two runaway `git diff` processes that had been consuming ~100% CPU each for 10+ hours — on the hypothesis that CPU contention, not a genuine hang, caused Blocker 3 above. **Confirmed:** the baseline (plan-only) `gold-run` for `doc:ent-q-syn-002-2024` completed cleanly this time (exit 0, real `extract_metrics`/`extract_provisions` LLM output, ~85s per provision — a plausible completion time, not a stall).

The first enforced-mode (`DOC_PIPELINE_PLAN_ONLY=false`) run against the same document came back **byte-identical in processor coverage** to the plan-only baseline — both `extract_metrics` and `extract_provisions` ran, despite `kb.doc_facets` confirming `input_doc_type='cdm'` for both records, which uniquely matches the seeded `benchmark-cdm-metrics-only` rule (priority 1, no conflicting rule) → pipeline `benchmark_metrics_only` (`Processors=[extract_metrics]`). Enforcement should have dropped `extract_provisions` and didn't. This was not a benchmark artifact — tracing `handleEvent` found a real bug:

`gold-run`'s event payload is `{"record_id": ..., "force": true}` — no `operation` field. `resolveProductionPlanFacts` builds `facts.RequestedProcessors` from `evt.Operations` alone, which is therefore empty. `applyPolicyFilter` computes `plan.ExcludedByPolicy()` by filtering `facts.RequestedProcessors` — an empty list filters to nothing, always. `applyPlanEnforcement` (the fix from earlier this session) filters the *real* running processor list (`s.Processors`, populated from `--processors`/the server's registered processors) against that always-empty `ExcludedByPolicy()` — a guaranteed no-op. The resolved pipeline was correct; the enforcement gate was checking the wrong (structurally empty) source. Critically, **`gold-run`'s payload shape matches the common real ingestion path** — most line-file-generated events don't carry an explicit per-event `operation` list either, they rely on the server's registered processor set — so this wasn't a `gold-run`-only quirk: enforcement was a silent no-op for that common case generally, not just for the benchmark. The earlier regression tests (`TestHandleEvent_EnforcedModeExcludesProcessorsNotInPipeline` etc.) all set an explicit `operation` list in their event payload, which is exactly what masked this.

**Fix** (`control.go`, `resolveProductionPlanFacts`): when `evt.Operations` is empty, fall back to the names of `s.Processors` — the real candidate list — as the `RequestedProcessors` baseline, instead of leaving it empty. This makes `plan.ExcludedByPolicy()` (and therefore `applyPlanEnforcement`, unchanged) reflect what's actually about to run, and keeps the persisted plan (`kb.doc_process_plans.excluded_by_policy`, `kb.doc_process_runs.parameters.processor_excluded_by_policy`) an accurate audit trail rather than a misleading always-empty one. Added `TestHandleEvent_EnforcedModeExcludesProcessorsNotInPipeline_NoExplicitOperations`, which reproduces `gold-run`'s exact payload shape (no `operation` field) and asserts both the real exclusion and the persisted `ExcludedByPolicy` agree; it fails without the fix and passes with it. Full `doc-processing` package test run afterward showed only pre-existing, unrelated failures (confirmed identical on the unmodified branch via `git stash`), none touching `control.go`/`handleEvent`.

Reran the enforced-mode `gold-run` with the fix in place. Result — a clean, real, end-to-end comparison on the same document with real (paid) LLM calls throughout:

| | Baseline (`DOC_PIPELINE_PLAN_ONLY` unset/`true`) | Enforced (`DOC_PIPELINE_PLAN_ONLY=false`) |
|---|---|---|
| Run / record | run 67 / record 96 | run 69 / record 98 |
| Resolved pipeline | *(plan-only: not gated)* | `benchmark_metrics_only`, via `rule_match` on `benchmark-cdm-metrics-only` |
| `extract_metrics` | ran, 1 metric extracted | ran, 1 metric extracted (unchanged — inside the pipeline's allowlist) |
| `extract_provisions` | ran, 1 provision extracted | **did not run** — `"extract_provisions":{"state":"rows","rows":[]}`, and no `extract_provisions start` log line at all |
| `kb.doc_process_plans.excluded_by_policy` (run 69) | n/a | `{extract_provisions}` |
| Exit code | 0 | 0 |

This is the Chunk 6 evidence deliverable: with no policy activated, both processors run (legacy-equivalent); with the seeded `benchmark_metrics_only`/`benchmark-cdm-metrics-only` policy activated and enforced, `extract_provisions` is genuinely excluded from execution — not just recorded as excluded — and the persisted plan (`kb.doc_process_plans.excluded_by_policy`) matches reality. **P1 benchmark closeout (Chunk 6) is now done.**

### `kb.pipeline_policies`: implemented (versioning wrapper), P1 closed out

Came back to the deferred-simplification item and built the scoped-down version the earlier decision called for: a real versioning/activation envelope, not the full DR6 schema (`scope_kind` generalization, `predicate JSONB`, per-processor `require|enable|skip|defer` gating remain deliberately out of scope — see the "scope: researched, treated as a deferred simplification" subsection above for why).

**Schema** (3 migrations, `20260731000011`–`13`): `kb.pipeline_policies(id, version, status, source_ref, checksum, activated_at, activated_by)`, status constrained to `draft|active|archived` with a partial unique index (`((1)) WHERE status = 'active'`) enforcing at most one active policy at the database level — verified this actually blocks a second simultaneous activation, not just documented as an invariant. `policy_id` added to both `kb.pipeline_bindings` and `kb.pipeline_rules` (`NOT NULL`, FK, backfilled to a seeded bootstrap policy so existing rows keep working with zero behavior change — same "no active policy means legacy behavior" invariant P1 has carried throughout). `kb.pipeline_bindings`'s uniqueness moved from `(ks_store_id)` to `(ks_store_id, policy_id)`, since the whole point is that the same store can now have a binding under an archived policy and a different one authored under a draft policy simultaneously.

**Resolution gating**: bindings are resolved live per-request (`extract-doc-metadata-store.go`'s `LEFT JOIN kb.pipeline_bindings`), rules are cached in-memory at process startup (`PipelineRuleSQLStore.ListPipelineRules`); both SQL paths now filter to `policy_id = (SELECT id FROM kb.pipeline_policies WHERE status = 'active' LIMIT 1)`, on top of whatever filtering already existed (`r.active` for rules). No new in-memory "current active policy" cache was needed — the live paths already reflect activation immediately, the cached path picks it up on next process restart (matching the pre-existing, still-deferred "no live-reload" limitation).

**Explainability**: `ProductionPlanFacts` gained `ActivePolicyID`/`ActivePolicyVersion`, populated in `resolveProductionPlanFacts` via a new `PolicyStore` on `ControlService` (`PipelinePolicySQLStore`, wired in `runtime.go`). `ProductionPipelineBindingResolution` carries the same two fields through to the persisted plan (`kb.doc_process_plans.pipeline_binding` JSONB) — no new migration needed there, since that column was already JSONB. This directly satisfies DR6's "every run writes an immutable execution plan... the policy version, the selected pipeline and why."

**CRUD**: `kb.pipeline_policies` gets `List`/`Create` (as draft, `version` auto-incremented via `MAX(version)+1`) and `Activate` (`POST .../:id/activate`, a transaction that archives whatever was active and activates the target — relies on the partial unique index as the real enforcement, not just transaction ordering). `kb.pipeline_bindings`/`kb.pipeline_rules`'s existing `Create` endpoints gained an optional `policy_id` field, defaulting to the currently active policy when omitted — this is what makes authoring-under-a-draft-then-activating actually usable end to end, not just plumbing with no way to exercise it.

**Live-Postgres validation** (`chenweb_test`): applied all 3 migrations via the app's real goose runner, confirmed the bootstrap policy seeded correctly and all 4 existing rules / 2 existing bindings backfilled to it. Then did the real end-to-end proof, not just schema checks: authored a binding for an already-bound store under a new draft policy pointing at a *different* pipeline than the active policy's binding — confirmed resolution still returned the active policy's pipeline while the new one was draft, activated the draft (via the same archive-then-activate transaction the API uses), confirmed resolution immediately switched to the newly-active policy's binding. Repeated the same before/after proof for rules (a draft-policy rule was invisible to the active rule set — which was empty, since the policy that got activated for the bindings test had no rules of its own — then became the only visible rule immediately after its own policy was activated). Also confirmed the one-active-policy constraint is real: manually tried to activate a second policy while one was already active and got a genuine `duplicate key value violates unique constraint` error, not just documentation. Cleaned up all validation-only rows and rules afterward and restored the bootstrap policy as active.

**Tests**: `TestPipelinePolicySQLStoreGetActivePolicy*` (happy path, no-active-policy error, nil-DB), `TestListPipelinePolicies*`/`TestCreatePipelinePolicy*`/`TestActivatePipelinePolicy*` (including the not-found-rolls-back-the-transaction case), plus updated the pre-existing `pipeline_bindings`/`pipeline_rules` handler and store tests for the new `policy_id` column and the active-policy SQL filter. Full `doc-processing` + `kbhandler` suites pass except the same pre-existing, unrelated failures confirmed via `git stash` earlier in this devdoc (summary/search/registry tests, none touching pipeline code).

**P1 is now done** against its own stated exit criteria, including the `kb.pipeline_policies` gap that was the last open item from the ADR's DR6 read. What remains deliberately out of scope for P1 (not gaps, explicit boundary decisions made and documented across this log): DR6's full schema beyond the versioning wrapper (`predicate JSONB`, `scope_kind` generalization, per-processor `require|enable|skip|defer` gating), real non-synthetic document validation, and live-reload of a running process's pipeline/policy config.
...
**P1 is done, as of this update (Friday, July 31, 2026, later revision).** Every item from the P0/P1 handoff, plus everything found necessary along the way: `kb.pipelines` (authored registry), `kb.pipeline_bindings` (replaced the free-text `default_pipeline` column), `kb.doc_facets` (persisted facet table), `DOC_PIPELINE_PLAN_ONLY` shadow-mode control, `pipeline_spec`/`ExcludedByPolicy`/`pipeline_processors_match_executed` threaded through all plan-inspection surfaces, `kb.pipeline_rules` (facet-driven selection with real conflict detection), two enforcement-gating fixes (`applyPlanEnforcement` genuinely changing what executes, and `resolveProductionPlanFacts` falling back to `s.Processors` so enforcement isn't a no-op on the common no-explicit-`operation` path), the P1 benchmark closeout (Chunk 6 — a real before/after comparison on real LLM output), and **`kb.pipeline_policies`** (versioning/activation wrapper — `policy_id` threaded through bindings and rules, resolution gated by the active policy, CRUD with a real `Activate` transaction, and live-Postgres proof that draft-then-activate genuinely changes what resolves). Every migration, CRUD path, and resolution path has been validated against live Postgres, not just unit-tested.

**Deliberately out of scope for P1** (explicit boundary decisions made and documented in this log, not gaps found and left):

1. **The rest of DR6's schema beyond the versioning wrapper** — `scope_kind` generalization beyond `knowledge_store`, `predicate JSONB` instead of flat match columns, per-processor `require|enable|skip|defer` gating in `kb.pipeline_rules`. Researched directly against the ADR (DR6, lines 339–427); decided to build only the versioning/activation piece, not the full schema. See the two `kb.pipeline_policies` subsections above (scope decision, then implementation) for the full reasoning.
2. **Real, non-synthetic document coverage.** Every document used across every slice's validation, including the benchmark closeout and the policy-activation live-DB proof, is gold-run synthetic corpus data. Extending validation to a database holding real documents (likely `miner`) was explicitly deferred earlier in this session.
3. **Live-reload of a running `doc-processor`/`doc-benchmark` process** when `kb.pipelines`/`kb.pipeline_bindings`/`kb.pipeline_rules`/`kb.pipeline_policies` change — the registry/rules load once at `NewProductionRuntime` construction, not on every event. Live binding lookups (per-request SQL) do reflect activation immediately; only the in-memory rule cache needs a restart.

**Minor, low-priority notes carried forward:** `kb.pipelines.name` is `UNIQUE` at the DB level, so duplicate-name `CreatePipeline`/`UpdatePipeline` calls fail with a generic 500 (no dedicated 409 handling — matches the rest of this codebase's CRUD surface, which doesn't special-case unique-constraint violations either). Migration file cleanup in the main ChenWeb worktree was reconfirmed clean this session.
