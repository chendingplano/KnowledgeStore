...
## 7. Next expected slice

As of the `pipeline_spec` threading slice above, the four original candidates from this section now stand as:

- ~~make store-bound selection real in live data~~ — **done** (`kb.knowledge_store.default_pipeline` + `GetInputRecord` join).
- ~~replace the current code-seeded pipeline registry with a richer named-pipeline/binding model~~ — **partially done**: pipeline *identity* is now authored in `kb.pipelines` and CRUD-editable via `/kb/pipelines`. Still open: a `kb.pipeline_bindings` table so store→pipeline binding is itself authored data, and `kb.pipeline_rules`/`kb.pipeline_policies` for facet-driven applicability.
- ~~thread the selected pipeline/spec through more API/dashboard surfaces so shadow-mode review can compare "selected pipeline" and "executed processors" side by side~~ — **done**: `pipeline_spec` now flows through all four surfaces (`kb.doc_process_plans`, run parameters, live status snapshot, plan-history APIs), plus a genuinely new `pipeline_processors_match_executed` diagnostic field that does the actual comparison instead of leaving it to the client.
- ~~add a first explicit shadow-mode control (`DOC_PIPELINE_PLAN_ONLY` or equivalent)~~ — **done**.

All four original Chunk-4/5-adjacent candidates from the July 31 handoff, plus `kb.pipeline_bindings` and `kb.doc_facets`, are now closed to the extent P1's plan-only scope calls for:

- ~~`kb.pipeline_bindings`~~ — **done**: replaces `kb.knowledge_store.default_pipeline` outright (dropped in the same migration), FK-enforced against both `kb.knowledge_store` and `kb.pipelines`, one binding per store (`UNIQUE(ks_store_id)`), CRUD-editable via `/kb/pipeline-bindings`.
- ~~`kb.doc_facets`~~ — **done**: persisted mirror of `ProductionRoutingFacets`, upserted on every successful plan-fact resolution, readable via `GET /kb/doc-facets?record_id=N`.
...
Live-reload of a running `doc-processor`/`doc-benchmark` process when `kb.pipelines` changes is also still open — the registry loads once at `NewProductionRuntime` construction.

Name validation note: `kb.pipelines.name` is UNIQUE at the DB level, so duplicate-name `CreatePipeline`/`UpdatePipeline` calls fail with a generic 500 (no dedicated 409/"already exists" handling was added — this matches the rest of the CRUD surface in this codebase, which does not special-case unique-constraint violations either).
