...
### Deliberately deferred beyond P2 (documented boundaries, not gaps)

These are explicit scope decisions recorded here so a later phase or a future handoff never mistakes P2 for "the whole ontology." Each item names what was deliberately not built, why, and where it lands.

1. **Assertions and evidence schema (`kb.semantic_assertions`, `kb.assertion_evidence`, `kb.assertion_relations`, `kb.semantic_decision_candidates`) — P3.** The assertion model, DR9 typed references, and normalized value columns are a separate phase. This is why `primary_class_term_id` (a derived projection) is not yet populated: classification-as-assertion (DR10) needs the assertion store.
2. **Phase D association (`normalize_assertions`, `associate_semantics`, `project_semantics`) — P3.** The spec §10 stages, candidate/decision lifecycle, deferred retry on dependency-fingerprint change, projection build/repair, and association telemetry.
3. **Keyword lexicon instantiation (`kb.keyword_*`) — P3.** The DR15/DR16 second kernel instantiation. The kernel itself (P2 chunk D) is family-parameterized and ready; the keyword family (surfaces → `kb.keyword_concepts`) is P3, shipped behind `KEYWORD_RESOLVER_MODE=observe` first. The merged DR16 spec that supersedes the two keyword specs is still unwritten and should land before that work.
4. **Profiles, profile rules, and review scopes (`kb.ontology_profiles`, `kb.ontology_profile_rules`, `kb.ontology_review_scopes`) — P4.** The pilot 4b domain module, the review-scope freeze, and the spec §12.3 finding decision procedure.
5. **DR5 stage-DAG planner consuming `Requires`/`Produces` — later.** P2 chunk F completed seam 1 (the `ProcessorSpec` DR5 fields + the registry), but the DAG/wave planner that actually gates on `Requires`/`Produces` is not built; the Phase A/B/C legacy path remains active.
6. **Full `semrules` predicate language — P5.** P2 chunk F shipped the seam (predicate evaluator + `RegisterOperator`), deliberately minimal; the flat-column pipeline-rule path stays active until P5 adopts JSONB predicates.
7. **SHACL/RDF/OWL/SKOS projection and the SQL-vs-SHACL parity gate — P7.** DR13 defers serialization/interchange; the profile-rule SHACL emitters are P4, the parity gate moved to P7.
8. **Live-reload of a running process when module/release/activation content changes — not in P2.** The loader seam (chunk B) loads active releases at process startup; consumers arrive in P3+ and live-reload can be revisited then.
