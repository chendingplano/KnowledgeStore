...
**Date:** 2026-07-24 (updated 2026-07-24 after first full bring-up on staging; updated 2026-07-28 to add the `create-admin` sysadmin-bootstrap tool, §2.5; updated 2026-09-23 to note that the China-box path (`mise build-server-linux` + `scripts/deploy-server-china.sh`) now automates the `config.toml`/`config/`/`docs/doc-templates` rsync below in addition to migrations/prompts/reviewer configs — see the note after the manual rsync block in §2.3, and `2026092301-devdoc-doc-processor-config-drift.md` for the incident that prompted it)
...
**On the China box specifically, the two rsync lines above (`config.toml`, `config`) are no longer a manual step** — as of 2026-09-23, `mise build-server-linux` stages both (plus `docs/doc-templates`) into the `/tmp/chenweb-deploy` payload alongside the binaries/migrations/prompts/reviewer configs, and `scripts/deploy-server-china.sh` installs them on every run (`config.toml` only if it differs from the box's copy; `config/` and `docs/doc-templates` mirrored with `--delete`, same as `prompts/`). This closes the exact gap that caused a real incident: the box's `config.toml` had gone stale after a 2026-09-17 commit moved `[doc-processing] required_processors` out of `config.toml` into `config.local.toml`-only, and since the routine binary-swap script never touched `config.toml`, the box kept running the pre-2026-09-17 processor list no matter how many times `config.local.toml` was edited and `doc-processor` restarted — see `2026092301-devdoc-doc-processor-config-drift.md`. This manual rsync block still applies as-is to the original Mac Mini target (`192.168.29.96`) and to bringing up any brand-new box from scratch.
